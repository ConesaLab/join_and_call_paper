...
**Figure typography and output paths** are centralized in `src/plotting/R/00_figure_config.R` and `src/plotting/R/02_themes.R`:
...
**Paths on Windows vs WSL:** read-level and config paths auto-resolve (`C:/data/nih` on Windows, `/mnt/c/data/nih` in WSL). Override if needed: `JOIN_AND_CALL_NIH_DATA` (NIH data root), `JOIN_AND_CALL_NIH_REPORTS` (`documenting_NIH/fabian` tree), `JOIN_AND_CALL_REPO` (this repository).
...
- **`ont_r10_sy5y_plots.Rmd`** — mirrors mouse `all_plots.Rmd` for IsoQuant, FLAIR, and Bambu only (no SIRV/TUSCO/StringTie/resource plots). Expects `reports/ont_r10/{isoquant,flair,bambu}_report` with **`SY5Y_class_df_list.RData`** (or `Bclass_df_list.RData` / `all_class_data.RData`). All multi-tool figures use a **2×2** layout: IsoQuant | FLAIR on top, Bambu | shared legend on bottom. Writes to `plot_creation_ont_r10/`, including:
  ...
  - `sy5y_upset_fl1.pdf`
  - UJC occurrence across replicates: `sy5y_comb_bar_fl1.pdf` / `sy5y_comb_fl_bar_fl1.pdf` (counts) and `sy5y_perc_comb_bar_fl1.pdf` (0–65%) / `sy5y_perc_comb_fl_bar_fl1.pdf` (0–100%)
...
- **`filter_level_ont_r10_sy5y_plots.Rmd`** — continuous TPM survival curves: `sy5y_tpm_lines_log.pdf`, `sy5y_tpm_lines_linear.pdf`, `sy5y_tpm_lines_legend.pdf`
- **`filter_level_line_plots.Rmd`** (mouse) — `tpm_lines_legend.pdf`, `brain_tpm_lines_log.pdf`, `kidney_tpm_lines_log.pdf`, `brain_tpm_lines_linear.pdf`, `kidney_tpm_lines_linear.pdf`
- **`read_level_ont_r10_sy5y_plots.Rmd`** — combined read-level figure: `sy5y_read_level_combined.pdf`
