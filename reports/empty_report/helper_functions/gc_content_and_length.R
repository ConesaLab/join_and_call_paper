...
gc_length <- function(g, a, a_format = "gtf"){
  ...
  chr2trans <- data.frame(
    chrom = annotation@seqnames,
    transcript_id = annotation@elementMetadata$transcript_id
  ) %>% 
    group_by(chrom, transcript_id) %>% 
    summarise(exons = n())
  chr2trans <- as.data.frame(chr2trans)
  #chr2trans <- chr2trans[!duplicated(chr2trans$transcript_id),]
  gclength <- merge(chr2trans, gclength, by = "transcript_id")
