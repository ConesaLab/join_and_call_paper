  ...
  for (method in names(src_dirs[[platform]])) {
    ...
    for (fl_filter_level in c(1L)) {
      results <- process_and_plot(
        ...
        upset_matrix_label_size = PAPER_MOUSE_UPSET_MATRIX_LABEL
      )
...
```{r source-data-capture}
# ---- Source Data capture: figures built from all_results ---------------------
# Sheets: F2, SF2, SF3, SF5, F5, SF6, F6, SF7, F7, SF8. One long tidy table per
# figure, binding the 10 per-(platform, method) panels with platform/tool cols.
# (Fragments persist to disk; write_source_data() assembles the workbook later.)

# Bind a per-(platform, method) panel frame into one long tidy table.
.sd_bind <- function(all_results, accessor, fl = "1") {
  rows <- list()
  for (platform in names(all_results)) {
    for (method in names(all_results[[platform]])) {
      res <- all_results[[platform]][[method]][[fl]]
...
# $data from a panel that may be a ggplot or a list of ggplots.
.sd_panel_data <- function(p) {
  if (inherits(p, "ggplot")) return(p$data)
  if (is.list(p) && length(p) && inherits(p[[1]], "ggplot")) return(p[[1]]$data)
  NULL
}

# Aggregate raw geom_bar(..count..) rows to the plotted per-category counts.
.sd_bar_counts <- function(df) {
  if (is.null(df) || !nrow(df)) return(NULL)
  dplyr::count(df, sample, structural_category, name = "n_isoforms")
}

# Relabel the raw `sample` codes (B1..B7 / K1..K7) to the displayed strategy /
# sample labels (J&C, C&J, B1..B5 / K1..K5), matching the figure x-axis.
sd_sample_map_brain  <- setNames(sample_labels_Brain, sample_codes_brain)
sd_sample_map_kidney <- setNames(sample_labels_Kidney, sample_codes_kidney)
.sd_relabel_sample <- function(df, map) {
  if (!is.null(df) && "sample" %in% names(df)) {
    s <- as.character(df$sample)
    hit <- s %in% names(map)
    s[hit] <- unname(map[s[hit]])
    df$sample <- s
  }
  df
}

# Aggregate the UJC membership matrix to plotted UpSet intersection sizes:
# count of UJCs per presence pattern (across sets) and structural category.
.sd_upset_counts <- function(df) {
  if (is.null(df) || !nrow(df)) return(NULL)
...
add_source_sheet("Fig 2",
  "Figure 2: number of transcripts per SQANTI3 structural category, per sample and combination strategy (brain).",
  .sd_relabel_sample(
    .sd_bind(all_results, function(res) .sd_bar_counts(.sd_panel_data(res$brain_plots))),
    sd_sample_map_brain))
add_source_sheet("Supplementary Fig 2",
  "Supplementary Fig. 2: number of transcripts per SQANTI3 structural category (kidney).",
  .sd_relabel_sample(
    .sd_bind(all_results, function(res) .sd_bar_counts(.sd_panel_data(res$kidney_plots))),
    sd_sample_map_kidney))

add_source_sheet("Supplementary Fig 3",
  "Supplementary Fig. 3: read (FL) counts per SQANTI3 structural category per sample, including unassigned reads (brain).",
  .sd_relabel_sample(
    .sd_bind(all_results, function(res) .sd_panel_data(res$brain_expression_plot)),
    sd_sample_map_brain))
add_source_sheet("Supplementary Fig 5",
  "Supplementary Fig. 5: read (FL) counts per SQANTI3 structural category per sample, including unassigned reads (kidney).",
  .sd_relabel_sample(
    .sd_bind(all_results, function(res) .sd_panel_data(res$kidney_expression_plot)),
    sd_sample_map_kidney))

add_source_sheet("Fig 5",
  "Figure 5: UJC intersection sizes (number of unique junction chains per presence pattern across samples/strategies) by structural category (brain).",
  .sd_bind(all_results, function(res) .sd_upset_counts(res$brain_upset_data)))
add_source_sheet("Supplementary Fig 6",
  "Supplementary Fig. 6: UJC intersection sizes by structural category (kidney).",
  .sd_bind(all_results, function(res) .sd_upset_counts(res$kidney_upset_data)))

add_source_sheet("Fig 6",
  "Figure 6: UJC distribution across number of samples, by structural category (brain).",
  .sd_bind(all_results, function(res) res$brain_perc_comb_bar_plot$data))
add_source_sheet("Supplementary Fig 7",
  "Supplementary Fig. 7: UJC distribution across number of samples (kidney).",
  .sd_bind(all_results, function(res) res$kidney_perc_comb_bar_plot$data))

add_source_sheet("Fig 7",
  "Figure 7: Read (FL) distribution of UJCs across number of samples, by structural category (brain).",
  .sd_bind(all_results, function(res) res$brain_perc_comb_fl_bar_plot$data))
add_source_sheet("Supplementary Fig 8",
  "Supplementary Fig. 8: Read (FL) distribution of UJCs across number of samples (kidney).",
  .sd_bind(all_results, function(res) res$kidney_perc_comb_fl_bar_plot$data))
```
...
brain_plot_1 <- assemble_bar_plots(all_results, "brain", 1, title="Transcripts per structural category; brain", subtitle="Filter: >=1 read", pb_ylims = pb_ylims, ont_ylims = ont_ylims, x_label = "Combination strategy / sample")
...
kidney_plot_1 <- assemble_bar_plots(all_results, "kidney", 1, title="Transcripts per structural category; kidney", subtitle="", pb_ylims = pb_ylims, ont_ylims = ont_ylims, x_label = "Combination strategy / sample")
...
brain_expr_plot_1 <- assemble_expr_bar_plots(all_results, "brain", 1, "Transcript expression per structural category; brain", pb_ylims = pb_ylims, ont_ylims = ont_ylims, plot_name_suffix="_expression_plot", use_scientific = TRUE, x_label = "Combination strategy / sample")
...
kidney_expr_plot_1 <- assemble_expr_bar_plots(all_results, "kidney", 1, "Transcript expression per structural category; kidney", pb_ylims = pb_ylims, ont_ylims = ont_ylims, plot_name_suffix="_expression_plot", use_scientific = TRUE, x_label = "Combination strategy / sample")
...
comb_bar_title <- "Unique Junction Chain occurrence across samples; brain"

brain_comb_bar_plot <- assemble_comb_plots(
  all_results, "brain", 1, comb_bar_title,
  pb_ylims = c(0, 35000),
  ont_ylims = c(0, 20000),
  plot_name_suffix = "_comb_bar_plot",
  y_label = PAPER_UJC_YLAB$comb_bar
)
paper_ggsave(
  file.path(out_dir, "brain_comb_bar.pdf"),
  plot = brain_comb_bar_plot,
  size = "mouse_10panel"
)

brain_perc_comb_bar_plot <- assemble_comb_plots(
  all_results, "brain", 1, comb_bar_title,
  pb_ylims = c(0, 65),
  ont_ylims = c(0, 65),
  plot_name_suffix = "_perc_comb_bar_plot",
  y_label = PAPER_UJC_YLAB$perc_comb_bar,
  x_label = "Number of samples present"
)
paper_ggsave(
  file.path(out_dir, "brain_perc_comb_bar.pdf"),
  plot = brain_perc_comb_bar_plot,
  size = "mouse_10panel"
)
...
comb_bar_title <- "Unique Junction Chain occurrence across samples; kidney"

kidney_comb_bar_plot <- assemble_comb_plots(
  all_results, "kidney", 1, comb_bar_title,
  pb_ylims = c(0, 35000),
  ont_ylims = c(0, 20000),
  plot_name_suffix = "_comb_bar_plot",
  y_label = PAPER_UJC_YLAB$comb_bar
...
paper_ggsave(
  file.path(out_dir, "kidney_comb_bar.pdf"),
  plot = kidney_comb_bar_plot,
  size = "mouse_10panel"
)

kidney_perc_comb_bar_plot <- assemble_comb_plots(
  all_results, "kidney", 1, comb_bar_title,
  pb_ylims = c(0, 65),
  ont_ylims = c(0, 65),
  plot_name_suffix = "_perc_comb_bar_plot",
...
paper_ggsave(
  file.path(out_dir, "kidney_perc_comb_bar.pdf"),
  plot = kidney_perc_comb_bar_plot,
  size = "mouse_10panel"
)
...
comb_fl_title <- "Expression of UJCs across samples; brain"

brain_comb_fl_bar_plot <- assemble_comb_plots(
  all_results, "brain", 1, comb_fl_title,
  pb_ylims = c(0, 20000000),
  ont_ylims = c(0, 20000000),
  plot_name_suffix = "_comb_fl_bar_plot",
  y_label = PAPER_UJC_YLAB$comb_fl_bar
)
paper_ggsave(
  file.path(out_dir, "brain_comb_fl_bar.pdf"),
  plot = brain_comb_fl_bar_plot,
  size = "mouse_10panel"
)

brain_perc_comb_fl_bar_plot <- assemble_comb_plots(
  all_results, "brain", 1, comb_fl_title,
  pb_ylims = c(0, 100),
  ont_ylims = c(0, 100),
  plot_name_suffix = "_perc_comb_fl_bar_plot",
  y_label = PAPER_UJC_YLAB$perc_comb_fl_bar,
  x_label = "Number of samples present"
)
paper_ggsave(
  file.path(out_dir, "brain_perc_comb_fl_bar.pdf"),
  plot = brain_perc_comb_fl_bar_plot,
  size = "mouse_10panel"
)
...
comb_fl_title <- "Expression of UJCs across samples; kidney"

kidney_comb_fl_bar_plot <- assemble_comb_plots(
  all_results, "kidney", 1, comb_fl_title,
  pb_ylims = c(0, 25000000),
...
paper_ggsave(
  file.path(out_dir, "kidney_comb_fl_bar.pdf"),
  plot = kidney_comb_fl_bar_plot,
  size = "mouse_10panel"
)

kidney_perc_comb_fl_bar_plot <- assemble_comb_plots(
  all_results, "kidney", 1, comb_fl_title,
  pb_ylims = c(0, 100),
  ont_ylims = c(0, 100),
  plot_name_suffix = "_perc_comb_fl_bar_plot",
...
paper_ggsave(
  file.path(out_dir, "kidney_perc_comb_fl_bar.pdf"),
  plot = kidney_perc_comb_fl_bar_plot,
  size = "mouse_10panel"
)
...
all_results$ONT$Bambu$`1`$brain_perc_comb_bar_plot$data
