...
# Includes an "Unaligned" category computed from read_numbers totals.
build_category_counts <- function(prefix, label_base, read_numbers) {
  ...
  # Total reads per sample from read_numbers
  rn <- read_numbers %>%
...
plot_sqanti_faceted <- function(counts_df, title = NULL) {
  ...
  extended_color <- c(setNames(rep(NA, length(cat.palette)), names(cat.palette)),
                      "Unaligned" = "black")
  ...
  ggplot(counts_df, aes(x = technology, y = num_reads,
                        fill = category_label, color = category_label)) +
    geom_col(width = 0.8, linewidth = 0.3, position = position_stack(reverse = TRUE)) +
    ...
    labs(x = NULL, y = "Number of reads", title = title) +
    theme(
...
# Build both tissues (includes Unaligned reads computed from read_numbers)
